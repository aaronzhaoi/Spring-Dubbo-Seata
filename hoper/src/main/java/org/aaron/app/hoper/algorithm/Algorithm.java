...
public class Algorithm {
     ...
     * 饿汉单例模式
     */
    private volatile static Algorithm algorithmLazy;

    private static Algorithm getInstanceLazy() {
        if (algorithmLazy == null) {
            synchronized (Algorithm.class) {
                if (algorithmLazy == null) {
                    algorithmLazy = new Algorithm();
                }
            }
        }
    ...
    public boolean findData2(int[][] searchData, int target) {
        int rowLength = searchData[0].length;
        int hangLength = searchData.length;
        int rows = 0;
        int hangs = 0;
        int rows1 = 0;
        int hangs1 = 0;
        int rows2 = 0;
        int hangs2 = 0;
        while (hangs1 <= hangLength && hangs2 <= hangLength && rows1 <= rowLength && rows2 <= rowLength) {
            if (searchData[rows][hangs] < target) {
                hangs1 = hangs++;
                rows1 = rows++;
            }
            if (searchData[rows][hangs] > target) {
            ...
            if (hangs + 1 == hangs2 && rows1 + 1 == rows2) {
                break;
            }
        }

        if (searchData[rows2][0] < target) {
            for (int i = 0; i < hangs2; i++) {
                if (searchData[rows2][i] == target) {
                    return true;
                }
            }
            // 二分查找
            ...
            while (true) {
                mid = (start + end) / 2;
                if (searchData[rows2][mid] > target) {
                    end = mid;
                }
                if (searchData[rows2][mid] < target) {
                    start = mid;
        ...
        if (searchData[rows1][hangLength] > target) {
            for (int i = hangs1; i < hangLength; i++) {
                if (searchData[rows1][i] == target) {
                    return true;
                }
            }
        }
    ...
    private static  void printStack(){
        ...
        System.out.print(integerStack.get(1));
  ...
  /*  public String longestCommonPrefix(String[] strs) {
    ...
    public void doMulThreadPools(Algorithm algorithm) throws ExecutionException, InterruptedException {
    ...
    public static void main(String[] args) throws ExecutionException, InterruptedException {
     /*   String[] str = {"asd", "asfda", "dsa", "12"};
        ...
        Algorithm.printListv2(stringList, stringListTest);*/
       /* String[] retStr = Algorithm.splitString("dsa");
        System.out.print("      ");*/
